...
const getRank = (card: string) => ranks.find(rank => card.toLowerCase().includes(rank)) as Rank;
const parseInput = (card: string) => ({ rank: getRank(card), suit: getSuit(card) });
...
export const compare = (cardInput1: string, cardInput2: string) => {
  const card1 = parseInput(cardInput1);
  const card2 = parseInput(cardInput2);
